...
var auth323 = require('../auth_323.js');
ClientHandshake.prototype.calculateOldPasswordAuthToken = function(
  authPluginData
) {
  return (authToken = auth323.scramble323(authPluginData, this.password));
};
...
ClientHandshake.prototype.handshakeResult = function(packet, connection) {
  ...
  if (marker === 0xfe || marker === 1) {
    ...
    if (authSwitchHandlerParams.pluginName == 'mysql_native_password') {
      ...
      connection.config.insecureAuth &&
      authSwitchHandlerParams.pluginName == 'mysql_old_password'
    ) {
      var authToken = this.calculateOldPasswordAuthToken(
        authSwitchHandlerParams.pluginData
      );
      connection.writePacket(
        new Packets.AuthSwitchResponse(authToken).toPacket()
